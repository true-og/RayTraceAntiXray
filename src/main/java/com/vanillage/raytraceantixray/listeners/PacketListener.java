...
public final class PacketListener extends PacketAdapter {
    ...
    public void onPacketSending(PacketEvent event) {
        ...
        if (event.getPacketType() == PacketType.Play.Server.MAP_CHUNK) {
            ...
            if (chunkBlocks == null) {
                ...
                if (!location.getWorld().equals(playerData.getLocations()[0].getWorld())) {
                    ...
                    playerData = new PlayerData(
                            plugin.getLocations(event.getPlayer(), new VectorialLocation(location)));
            ...
            plugin.getPlayerData().get(event.getPlayer().getUniqueId()).getChunks()
                    .remove(new LongWrapper(ChunkPos.asLong(integers.read(0), integers.read(1))));
            ...
            // Technically this isn't necessary since we're using a weak reference to the
            // chunk.
