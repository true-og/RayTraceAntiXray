    ...
    public void onWorldInit(WorldInitEvent event) {
        ...
        if (plugin.isEnabled(world)) {
            ...
            List<String> rayTraceBlocks = config
                    ...
                    .stream().filter(o -> o != null).map(String::valueOf).collect(Collectors.toList());
            Level level = ((CraftWorld) world).getHandle();
            Object controller = new ChunkPacketBlockControllerAntiXray(plugin, rayTraceThirdPerson, rayTraceDistance,
                    maxRayTraceBlockCountPerChunk, rehideBlocks, rayTraceBlocks.isEmpty() ? null : rayTraceBlocks,
                    level, MinecraftServer.getServer().executor);
